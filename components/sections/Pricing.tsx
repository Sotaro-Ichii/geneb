...
export default function Pricing() {
  ...
  return (
    <Section className="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 relative overflow-hidden py-16 sm:py-24 lg:py-32" id="pricing">
      ...
      <Container className="relative z-10">
        ...
        <div className="mb-12 sm:mb-16">
          <h3 className="text-2xl sm:text-3xl font-black text-slate-900 mb-6 sm:mb-8 text-center">
                ...
                <Card key={plan.id} className="flex-shrink-0 w-72 sm:w-80 relative border-0 shadow-xl hover:shadow-2xl transition-all duration-500 bg-white/90 backdrop-blur-md border border-blue-200 snap-start group hover:scale-105">
                  ...
                  <CardHeader className="text-center pb-3 sm:pb-4 pt-6 sm:pt-4">
                    ...
                    <div className="mt-2 sm:mt-3 space-x-2">
                      ...
                      <span className="text-2xl sm:text-3xl font-black text-rose-600">¥{yen.format(plan.campaignPrice)}</span>
                    </div>
                    <p className="text-xs text-slate-600 mt-1 sm:mt-2">{plan.target}</p>
                  </CardHeader>
                  <CardContent className="pt-2 sm:pt-4 flex flex-col grow">
                    <ul className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                      {plan.features.map((f, i) => (
                        <li key={i} className="flex items-start gap-2 sm:gap-3">
                          <Check className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-600 mt-0.5 flex-shrink-0" />
                    ...
                    <Link href={SITE_CONFIG.contactUrl} data-plan-id={plan.id} data-plan-name={plan.name} data-price={plan.campaignPrice} className="mt-auto block">
                      <Button className="w-full bg-gradient-to-r from-rose-500 to-pink-500 hover:from-rose-600 hover:to-pink-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-500 hover:scale-105 text-sm sm:text-base" aria-label={`${plan.name}を相談する`} onClick={() => trackEvent('cta_production_click','engagement', `${plan.id}_${plan.name}`, plan.campaignPrice)}>
                        このプランで相談する
                      </Button>
                    </Link>
                  </CardContent>
        ...
        {/* 2) 月額プラン（維持・更新） */}
        <div className="mt-12 sm:mt-16 lg:mt-20 mb-12 sm:mb-16">
          <h3 className="text-2xl sm:text-3xl font-black text-slate-900 mb-6 sm:mb-8 text-center">
                ...
                <Card key={plan.id} className="flex-shrink-0 w-72 sm:w-80 relative border-0 shadow-xl hover:shadow-2xl transition-all duration-500 bg-white/90 backdrop-blur-md border border-blue-200 snap-start group hover:scale-105">
                  ...
                  <CardHeader className="text-center pb-3 sm:pb-4 pt-6 sm:pt-4">
                    ...
                    <div className="mt-2 sm:mt-3">
                      ...
                      <span className="text-slate-500 ml-1 text-xs sm:text-sm">/月</span>
                    </div>
                    {plan.target && (
                      <p className="text-xs text-slate-600 mt-1 sm:mt-2">{plan.target}</p>
                    )}
                  ...
                  <CardContent className="pt-2 sm:pt-4 flex flex-col grow">
                    <ul className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                      {plan.includes.map((inc, i) => {
                        ...
                        return (
                          <li key={i} className={`flex items-start gap-2 sm:gap-3 ${highlightGold || highlightPlat ? 'bg-yellow-50 rounded-lg px-2 sm:px-3 py-1 sm:py-2' : ''}`}>
                            <Check className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-600 mt-0.5 flex-shrink-0" />
                            <span className="text-xs sm:text-sm text-slate-700">{inc}</span>
                          </li>
                        );
                      })}
                    </ul>
                    <Link href={SITE_CONFIG.contactUrl} data-plan-id={plan.id} data-plan-name={plan.name} data-price={plan.priceMonthly} className="mt-auto block">
                      <Button className="w-full bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-500 hover:scale-105 text-sm sm:text-base" aria-label={`${plan.name}月額で相談する`} onClick={() => trackEvent('cta_monthly_click','engagement', `${plan.id}_${plan.name}`, plan.priceMonthly)}>
                        この月額で相談する
                      </Button>
                    </Link>
                  </CardContent>
        ...
        <div className="mt-12 sm:mt-16 lg:mt-20">
          ...
          <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
            <Card className="border-0 shadow-xl bg-white/90 backdrop-blur-md border border-blue-200 group hover:-translate-y-2 transition-all duration-500">
              <CardHeader>
                <h4 className="text-xl sm:text-2xl font-bold text-slate-900 group-hover:text-emerald-700 transition-colors duration-500">オプション</h4>
              </CardHeader>
              <CardContent>
                <ul className="space-y-2 sm:space-y-3 mb-6 sm:mb-8">
                  {pricing.options.map((opt, i) => (
                    <li key={i} className="flex items-start gap-2 sm:gap-3">
                      <Check className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600 mt-0.5 flex-shrink-0" />
                      <span className="text-sm text-slate-700">{opt}</span>
                    </li>
                  ))}
                </ul>
                <Link href={SITE_CONFIG.contactUrl} data-plan-id="options" data-plan-name="オプション" data-price="-">
                  <Button variant="secondary" className="w-full bg-gradient-to-r from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 text-white font-bold rounded-xl shadow-lg transition-all duration-500 hover:scale-105 text-sm sm:text-base" aria-label="オプションについて相談する" onClick={() => trackEvent('cta_options_click','engagement','options')}>
                    オプションについて相談する
                  </Button>
                </Link>
              </CardContent>
            ...
            <Card className="border-0 shadow-xl bg-white/90 backdrop-blur-md border border-blue-200 group hover:-translate-y-2 transition-all duration-500" id="annual-perk">
              <CardHeader>
                <h4 className="text-xl sm:text-2xl font-bold text-slate-900 group-hover:text-emerald-700 transition-colors duration-500">{pricing.annualContractPerk.title}</h4>
          ...
          <p className="text-xs text-slate-500 mt-6 sm:mt-8 text-center">{SITE_CONFIG.taxNote || pricing.taxNote}</p>
        </div>
      </Container>
    </Section>
  );
